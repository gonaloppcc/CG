...
template<typename T>
struct Point { // Note: This struct is also used to represent a vector if needed.
    T x;
    T y;
    T z;
};
...
void computeNormal(int i, int j) {
    // TODO: Careful with i = 0 or j = 0 or i = max or j = max

    // fill the normal vector with the normal for vertex at grid location (i,j)

    // p vertices
    Point<float> p1 = {static_cast<float>(i), h(i, j - 1), j - 1.f};
    Point<float> p2 = {static_cast<float>(i), h(i, j + 1), j + 1.f};
    Point<float> p3 = {i - 1.f, h(i - 1, j), static_cast<float>(j)};
    Point<float> p4 = {i + 1.f, h(i + 1, j), static_cast<float>(j)};

    // v1 and v2 vectors
    float v1[3] = {p2.x - p1.x, p2.y - p1.y, p2.z - p1.z};
    float v2[3] = {p4.x - p3.x, p4.y - p3.y, p4.z - p3.z};
